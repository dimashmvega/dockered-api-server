...
import { Between, DeleteResult, FindManyOptions, IsNull, LessThanOrEqual, MoreThanOrEqual, Not, Repository } from "typeorm";
import { PaginatedProducts, ProductFilterParams, ReportFilters, ProductMetrics, InventoryHealthReport } from "src/interfaces/productFilter.interface";
...
export class ProductService {
    ...
    async insertProduct(data: any): Promise<boolean> {
        try {            
            ...
            console.log(`Product SKU ${mappedProductData.sku} inserted/updated successfully.`);
            return true;
        } catch (error) {            
    ...
    async getAllProducts(filters: ProductFilterParams = {}): Promise<PaginatedProducts> {
        const page = (typeof filters.page === 'number' && filters.page > 0) ? filters.page : 1;
        const limit = (typeof filters.limit === 'number' && filters.limit > 0) ? filters.limit : DEFAULT_PAGE_SIZE;
        ...
        if (filters.minPrice || filters.maxPrice) {
            ...
            if (filters.maxPrice) {                
                priceFilter.price = { ...priceFilter.price, ...LessThanOrEqual(filters.maxPrice) };
            }
    ...
    async getProductMetrics(filters: ReportFilters = {}): Promise<ProductMetrics> {
        const totalRecords = await this.productRepository.count({ withDeleted: true });

        if (totalRecords === 0) {
            return { totalRecords: 0, deletedPercentage: 0, activeFilteredPercentage: 0 };
        }
    ...
    async getInventoryHealthReport(category?: string): Promise<InventoryHealthReport[]> {
        ...
        const query = this.productRepository.createQueryBuilder('p')
            ...
            .addSelect('AVG(EXTRACT(EPOCH FROM (NOW() - p."createdAt")) / 86400)', 'averageAgeDays')
